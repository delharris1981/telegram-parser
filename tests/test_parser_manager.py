...
from parser.manager import start_parser, stop_parser, parser_status, rename_parser
...
@pytest.mark.asyncio
async def test_rename_parser_rekeys_running_task():
    async def fake_loop(username, db_path):
        await asyncio.sleep(999)

    with patch("parser.manager.run_parser_loop", side_effect=fake_loop), \
         patch("parser.manager.init_db", new=AsyncMock()):
        await start_parser("carol", "data/carol/telelistener.db")
        await rename_parser("carol", "carolyn", "data/carol/telelistener.db")
        assert parser_status("carol") == "stopped"
        assert parser_status("carolyn") == "running"
        await stop_parser("carolyn")


@pytest.mark.asyncio
async def test_rename_parser_noop_when_not_running():
    await rename_parser("dave", "david", "data/dave/telelistener.db")
    assert parser_status("dave") == "stopped"
    assert parser_status("david") == "stopped"
